...
Scenario: To view the City's 'Basic Info' under the City tab so that a value can be entered that is different from current value for 'Add Info' field(Front End Validation)
...
Examples:
|country|area|city|addInfoText|addDifferentText|
...
Then the user should be able to verify the values are entered in the add info field
Then the user should see the successful update message at top of the page
Then the user should see different <addDifferentInfoText> value as in zeus document
Then the user reverts the changes to the document
...
Scenario: To view that there is no change in value when the user has entered a value for 'Add Info' that is no different to the current value(Back End Validation)
Given a user is on the search page
When the user clicks on the data tab in the search page
And the user clicks on the city tab in the data area
When the user clicks on the choose a country option

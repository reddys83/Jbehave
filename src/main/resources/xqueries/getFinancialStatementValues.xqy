...
let $financialStatement :=  /financialStatement[@source = $source and owner/link/@href = fn:concat("/legalEntity/id/",$legalEntityId[1]) and periodEnd = $periodEndDate]
let $auditedBy  := if($financialStatement/audited) then
    ...
    let $legalEntity := /legalEntity[@source = $source and @id=$legalEntityId]
...
let $document := /document[@source = $source and @id=$documentId]

...
let $offices := (for $x in cts:search(fn:collection('source-trusted')/office,
        ...
        ))) order by  $x/locations/location[@primary="true"]/address/area/name descending return $x) [1 to 25]

let $officeResults := (
    let $officeArea := for $x in $offices
    return <name>{$x/@fid}{$x/locations/location[@primary="true"]/address/area/name/text()}</name>
    return $officeArea)

...
let $entityLink := $areaDoc/relatedPresences/relation/presence/link/@href
let $relatedEntity := if(contains($entityLink, "legalEntity"))
                    then (
                       let $id := tokenize($entityLink, "/")[last()]
                       return /legalEntity[@source = $source and @id eq $id]/summary/names/name[type="Legal Title"]/value/text()
                    ) else (
                       let $id := tokenize($entityLink, "/")[last()]
                       return /office[@source = $source and @id eq $id]/summary/names/name[type="Legal Title"]/value/text()
                    )
let $entityType := ($areaDoc/relatedPresences/relation/type/text())
let $entityDetail := ($areaDoc/relatedPresences/relation/details/detail/text())
